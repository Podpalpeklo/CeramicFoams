...
%% GENETRATORPENOVESTRUKTURY Třída zajišťující práci s jádry pro Voroného teselaci ve 3D
%     Verze: 2016-10-23
%     Kontakt: Roman Papšík <roman.papsik@vutbr.cz>
	...
	methods (Access = public)
		...
		%% GENERATORPENOVESTRUKTURY Konstruktor třídy (nepotřebuje žádné parametry)
		%     Použití: instance = GeneratorPenoveStruktury

		end

		function VygenerovatJadra(gps, rozmery, velikostBunky, pravidelnost)
		%% VYGENEROVATJADRA Vygenetruje v prostoru jádra dle zadaných parametrů
		%     instance.VYGENEROVATJADRA(rozmery, velikostBunky, pravidelnost)
		%         rozmery - vektor 1x3
		%         velikostBunky - průměr koule buňce vepsané
		%         pravidelnost - hodnota v procentech
			...
			disp(['  * velikost buňky [', num2str(gps.velikostBunky), ']']);
		end

		function NacistJadra(gps, varargin)
		%% NACISTJADRA Načte jádra ze zvoleného souboru
		%     instance.NACISTJADRA - zobrazi dialogové okno pro výběr souboru
		%     instance.NACISTJADRA('C:\soubor.xyz') - načte soubor přímo

			if nargin == 2
				adresa = fullfile(varargin{1});
			% pokud nebyl zadany soubor, otevre se dialog
			else
				[jmeno_souboru, adresar_souboru] = uigetfile({'*.xyz', 'Souřadnice jader'}, 'Načíst soubor s jádry');
				% sobor nebyl zvolen
		...
		%     instance.ULOZITJADRA - zobrazi dialogové okno pro výběr souboru
		%     instance.ULOZITJADRA('C:\soubor.xyz') - uloží soubor přímo

			% oveření, zda nějaká jádra existují
			...
			% pokud nebyl zadany soubor, otevre se dialog
				...
				[jmeno_souboru, adresar_souboru] = uiputfile({'*.xyz', 'Souřadnice jader'}, 'Uložit soubor s jádry');
				% sobor nebyl zvolen
				if isequal(jmeno_souboru, 0) || isequal(adresar_souboru, 0)
					warning('Soubor nebyl vybrán');
					return;
		...
		%     instance.VYTVORITAPDL(vystupniSoubor) - Parametrem je adresa a název výstupního souboru
			...
			disp('Vytváření voroného teselace a APDL kódu...');
	...
	methods (Access = protected)
		...
		%% VYGENEROVATPRAVIDELNE Vytvoří zcela pravidelnou strukturu BCC mřížky
		...
		%% VYGENEROVATNAHODNE Umisťuje náhodně do prostoru jádra, pokud se v jejich okolí nevyskytují jiné
		...
		%% ROZMISTIT Vychýlí jádra z pozic určených zcela pravidelnou strukturou
		...
		%% MINIMALNIVZDALENOST Vypočítá nejmenší vzdálenost mezi jádry při dokonalém uspořádání
		...
		%     instance.OVERENIVZDALENOSTI([x,y,z]) - parametrem je vektor 3 souřadnic v prostoru
			if isempty(gps.jadra)
				prijmout = true;
				return;
			end
